...
function DashboardPage() {
  ...
  return (
    <div className="relative space-y-8">
      ...
      <div className="pointer-events-none fixed inset-0 bg-[linear-gradient(to_right,hsl(var(--border)/0.3)_1px,transparent_1px),linear-gradient(to_bottom,hsl(var(--border)/0.3)_1px,transparent_1px)] bg-[size:24px_24px] [mask-image:radial-gradient(ellipse_80%_50%_at_50%_0%,#000_40%,transparent_100%)]" />
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatsCard
          ...
          icon={Target}
          variant="default"
          trend={completionRate >= 50 ? { value: "on track", positive: true } : undefined}
        />
      </div>
            ...
            {pendingCount > 0 ? (
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-muted-foreground">
                  <span className="font-medium text-foreground">{pendingCount}</span> pending {pendingCount === 1 ? "task" : "tasks"}
                </span>
              </div>
            ) : totalCount > 0 ? (
