...
import { Users, Activity, CheckCircle2, TrendingUp, MoreHorizontal } from "lucide-react";
    ...
    <div className="grid gap-4 md:grid-cols-3">
      ...
      {stats.map((stat, index) => (
        ...
        >
          ...
          <div className="mt-2 text-2xl font-bold text-card-foreground">{stat.value}</div>
          <div className={`mt-1 text-xs ${
            stat.changeType === "positive" ? "text-emerald-600" : "text-violet-600"
          }`}>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <div>
            <span className="text-sm font-medium text-card-foreground">Activity Overview</span>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          ...
          <button className="text-xs text-violet-600 hover:text-violet-700 font-medium">View all</button>
        </div>
          ...
          {activities.map((item, i) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm text-card-foreground truncate">{item.name}</div>
